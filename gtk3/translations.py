...
_T: dict[str, dict[str, str]] = {
    ...
    "tunnel.close_warning_title": {"it": "Tunnel SSH attivi", "en": "Active SSH tunnels", "de": "Aktive SSH-Tunnel", "fr": "Tunnels SSH actifs", "es": "Túneles SSH activos"},
    "tunnel.close_warning_msg":   {"it": "Ci sono <b>{n}</b> tunnel SSH attivi:\n\n{names}\n\nCosa vuoi fare alla chiusura di PCM?", "en": "There are <b>{n}</b> active SSH tunnel(s):\n\n{names}\n\nWhat do you want to do when closing PCM?", "de": "Es gibt <b>{n}</b> aktive SSH-Tunnel:\n\n{names}\n\nWas möchten Sie beim Schließen von PCM tun?", "fr": "Il y a <b>{n}</b> tunnel(s) SSH actif(s) :\n\n{names}\n\nQue voulez-vous faire à la fermeture de PCM ?", "es": "Hay <b>{n}</b> túnel(es) SSH activo(s):\n\n{names}\n\n¿Qué deseas hacer al cerrar PCM?"},
    "tunnel.close_keep":          {"it": "🔌  Lascia i tunnel attivi", "en": "🔌  Keep tunnels running", "de": "🔌  Tunnel aktiv lassen", "fr": "🔌  Garder les tunnels actifs", "es": "🔌  Dejar los túneles activos"},
    "tunnel.close_stop":          {"it": "✖  Chiudi e ferma i tunnel", "en": "✖  Close and stop tunnels", "de": "✖  Schließen und Tunnel beenden", "fr": "✖  Fermer et arrêter les tunnels", "es": "✖  Cerrar y detener los túneles"},
    "tunnel.close_cancel":        {"it": "Annulla", "en": "Cancel", "de": "Abbrechen", "fr": "Annuler", "es": "Cancelar"},
    ...
    "sd.totp_secret":  {"it": "Secret TOTP:",    "en": "TOTP secret:",    "de": "TOTP-Geheimnis:",  "fr": "Secret TOTP :",   "es": "Secreto TOTP:"},
    "sd.tags":         {"it": "Tag:",             "en": "Tags:",           "de": "Tags:",            "fr": "Tags :",          "es": "Etiquetas:"},
    ...
    "sd.save_vnc_password_failed": {"it": "Impossibile salvare la password VNC.", "en": "Unable to save the VNC password.", "de": "Das VNC-Passwort konnte nicht gespeichert werden.", "fr": "Impossible d'enregistrer le mot de passe VNC.", "es": "No se pudo guardar la contraseña VNC."},
    ...
    "crypto.first_run.description":     {"it": "PCM può cifrare utenti e password in connections.json con AES-128. Ad ogni avvio viene chiesta la password master per sbloccare.\n\nSenza cifratura le credenziali restano in chiaro nel JSON.", "en": "PCM can encrypt saved credentials in connections.json using AES-128. At startup the master password will be requested.\n\nWithout encryption, credentials remain in plain text.", "de": "PCM kann Anmeldedaten in connections.json mit AES-128 verschlüsseln. Beim Start wird das Masterpasswort abgefragt.\n\nOhne Verschlüsselung bleiben Daten im Klartext.", "fr": "PCM peut chiffrer les identifiants dans connections.json avec AES-128. Au démarrage le mot de passe maître sera demandé.\n\nSans chiffrement, les identifiants restent en clair.", "es": "PCM puede cifrar credenciales en connections.json con AES-128. Al inicio se pedirá la contraseña maestra.\n\nSin cifrado quedan en texto plano."},
...
# ── Gestione Cifratura (Dialog) ──────────────────────────────
    ...
    "ftp.label_tls_cert":   {"it": "Certificato TLS:", "en": "TLS certificate:", "de": "TLS-Zertifikat:", "fr": "Certificat TLS :", "es": "Certificado TLS:"},
    "ftp.label_tls_key":    {"it": "Chiave privata TLS:", "en": "TLS private key:", "de": "TLS-Privatschlüssel:", "fr": "Clé privée TLS :", "es": "Clave privada TLS:"},
    "ftp.err_no_users":     {"it": "Configura almeno un utente prima di avviare il server.", "en": "Configure at least one user before starting the server.", "de": "Konfigurieren Sie mindestens einen Benutzer, bevor Sie den Server starten.", "fr": "Configurez au moins un utilisateur avant de démarrer le serveur.", "es": "Configure al menos un usuario antes de iniciar el servidor."},
    "ftp.err_user_credentials": {"it": "Gli utenti nominativi richiedono nome e password.", "en": "Named users require a username and password.", "de": "Benannte Benutzer benötigen Benutzername und Passwort.", "fr": "Les utilisateurs nommés exigent un nom et un mot de passe.", "es": "Los usuarios con nombre requieren usuario y contraseña."},
    "ftp.err_tls_files":    {"it": "Seleziona un certificato e una chiave privata TLS validi.", "en": "Select a valid TLS certificate and private key.", "de": "Wählen Sie ein gültiges TLS-Zertifikat und einen privaten Schlüssel.", "fr": "Sélectionnez un certificat TLS et une clé privée valides.", "es": "Seleccione un certificado TLS y una clave privada válidos."},
    ...
    "tt.totp_secret": {"it": "Secret TOTP (Time-based One-Time Password) per l'autenticazione a due fattori. Puoi incollare un secret base32 (es. JBSWY3DPEHPK3PXP) o un URI otpauth://. PCM genererà il codice OTP automaticamente dopo la password. Funziona con Google Authenticator, Authy e altri", "en": "TOTP (Time-based One-Time Password) secret for two-factor authentication. Paste a base32 secret (e.g. JBSWY3DPEHPK3PXP) or an otpauth:// URI. PCM will generate the OTP code automatically after the password. Works with Google Authenticator, Authy and others", "de": "TOTP-Geheimnis (Time-based One-Time Password) für Zwei-Faktor-Authentifizierung. Base32-Geheimnis (z.B. JBSWY3DPEHPK3PXP) oder otpauth:// URI einfügen. PCM generiert den OTP-Code automatisch nach dem Passwort. Funktioniert mit Google Authenticator, Authy und anderen", "fr": "Secret TOTP (Time-based One-Time Password) pour l'authentification à deux facteurs. Collez un secret base32 (ex. JBSWY3DPEHPK3PXP) ou un URI otpauth://. PCM générera le code OTP automatiquement après le mot de passe. Compatible avec Google Authenticator, Authy et autres", "es": "Secreto TOTP (Time-based One-Time Password) para autenticación de dos factores. Pegue un secreto base32 (ej. JBSWY3DPEHPK3PXP) o un URI otpauth://. PCM generará el código OTP automáticamente después de la contraseña. Compatible con Google Authenticator, Authy y otros"},
    "tt.tags":        {"it": "Tag separati da virgola per categorizzare la sessione (es. prod,database,critico). Usa il filtro tag sotto la barra di ricerca nella sidebar per filtrare le sessioni", "en": "Comma-separated tags to categorise the session (e.g. prod,database,critical). Use the tag filter below the search bar in the sidebar to filter sessions", "de": "Durch Kommas getrennte Tags zur Kategorisierung der Sitzung (z.B. prod,datenbank,kritisch). Verwenden Sie den Tag-Filter unter der Suchleiste, um Sitzungen zu filtern", "fr": "Tags séparés par des virgules pour catégoriser la session (ex. prod,base_de_données,critique). Utilisez le filtre de tags sous la barre de recherche pour filtrer les sessions", "es": "Etiquetas separadas por comas para categorizar la sesión (ej. prod,base_de_datos,crítico). Use el filtro de etiquetas debajo de la barra de búsqueda para filtrar sesiones"},
    "tt.template":    {"it": "Seleziona una sessione template. I campi del template (protocollo, host, utente, password, chiave, secret TOTP) verranno copiati nei campi vuoti di questa nuova sessione. I campi già compilati non verranno sovrascritti", "en": "Select a template session. Template fields (protocol, host, user, password, key, TOTP secret) will be copied into empty fields of this new session. Already filled fields will not be overwritten", "de": "Wählen Sie eine Vorlagensitzung. Vorlagenfelder (Protokoll, Host, Benutzer, Passwort, Schlüssel, TOTP-Geheimnis) werden in leere Felder dieser neuen Sitzung kopiert. Bereits ausgefüllte Felder werden nicht überschrieben", "fr": "Sélectionnez une session modèle. Les champs du modèle (protocole, hôte, utilisateur, mot de passe, clé, secret TOTP) seront copiés dans les champs vides de cette nouvelle session. Les champs déjà remplis ne seront pas écrasés", "es": "Seleccione una sesión plantilla. Los campos de la plantilla (protocolo, host, usuario, contraseña, clave, secreto TOTP) se copiarán en los campos vacíos de esta nueva sesión. Los campos ya rellenados no se sobrescribirán"},
    "tt.template_checkbox": {"it": "Rendi questa sessione disponibile come template per altre sessioni. Le sessioni template appaiono nel menu a tendina 'Template' quando crei nuove sessioni", "en": "Make this session available as a template for other sessions. Template sessions appear in the 'Template' dropdown when creating new sessions", "de": "Machen Sie diese Sitzung als Vorlage für andere Sitzungen verfügbar. Vorlagensitzungen erscheinen im 'Template'-Dropdown beim Erstellen neuer Sitzungen", "fr": "Rendre cette session disponible comme modèle pour d'autres sessions. Les sessions modèles apparaissent dans le menu déroulant 'Modèle' lors de la création de nouvelles sessions", "es": "Hacer que esta sesión esté disponible como plantilla para otras sesiones. Las sesiones plantilla aparecen en el menú desplegable 'Plantilla' al crear nuevas sesiones"},
    ...
    "sidebar.tag_filter_all": {"it": "— tutti i tag —", "en": "— all tags —", "de": "— alle Tags —", "fr": "— tous les tags —", "es": "— todas las etiquetas —"},
    ...
    "importer.moba_title":    {"it": "MobaXterm (.mxtsessions / .mobaconf)", "en": "MobaXterm (.mxtsessions / .mobaconf)", "de": "MobaXterm (.mxtsessions / .mobaconf)", "fr": "MobaXterm (.mxtsessions / .mobaconf)", "es": "MobaXterm (.mxtsessions / .mobaconf)"},
    "importer.moba_none":     {"it": "Nessuna sessione trovata nel file MobaXterm", "en": "No sessions found in MobaXterm file", "de": "Keine Sitzungen in MobaXterm-Datei gefunden", "fr": "Aucune session trouvée dans le fichier MobaXterm", "es": "No se encontraron sesiones en el archivo MobaXterm"},
    "importer.moba_warn":     {"it": "⚠ Import parziale: password e percorsi chiavi Windows non vengono importati — verificare le sessioni dopo l'import", "en": "⚠ Partial import: passwords and Windows key paths are not imported — review sessions after import", "de": "⚠ Teilimport: Passwörter und Windows-Schlüsselpfade werden nicht importiert — Sitzungen nach dem Import überprüfen", "fr": "⚠ Import partiel : les mots de passe et chemins de clés Windows ne sont pas importés — vérifier les sessions après l'import", "es": "⚠ Importación parcial: las contraseñas y rutas de claves Windows no se importan — revisar las sesiones tras el import"},
    ...
    "mon.running_procs":        {"it": "Running Processes",        "en": "Running Processes",     "de": "Laufende Prozesse",       "fr": "Processus en cours",       "es": "Procesos en ejecución"},
    "mon.disk_usage":           {"it": "Disk Usage",               "en": "Disk Usage",            "de": "Festplattennutzung",      "fr": "Utilisation du disque",    "es": "Uso del disco"},
    "mon.network_usage":        {"it": "Network Usage",            "en": "Network Usage",         "de": "Netzwerknutzung",         "fr": "Utilisation réseau",       "es": "Uso de red"},
    "mon.col_path":             {"it": "Percorso",                 "en": "Path",                  "de": "Pfad",                    "fr": "Chemin",                   "es": "Ruta"},
    "mon.col_size":             {"it": "Totale",                   "en": "Total",                 "de": "Gesamt",                  "fr": "Total",                    "es": "Total"},
    "mon.col_used":             {"it": "Usato",                    "en": "Used",                  "de": "Belegt",                  "fr": "Utilisé",                  "es": "Usado"},
    "mon.col_pct":              {"it": "Uso%",                     "en": "Use%",                  "de": "Nutzung%",                "fr": "Util.%",                   "es": "Uso%"},
    "mon.kill_confirm":         {"it": "Terminare processo {pid}?","en": "Terminate process {pid}?","de": "Prozess {pid} beenden?","fr": "Terminer le processus {pid}?","es": "¿Terminar proceso {pid}?"},
    "mon.no_session":           {"it": "Nessuna sessione",         "en": "No session",            "de": "Keine Sitzung",           "fr": "Aucune session",           "es": "Sin sesión"},
    "mon.connecting":           {"it": "Connessione in corso…",    "en": "Connecting…",           "de": "Verbinde…",               "fr": "Connexion en cours…",      "es": "Conectando…"},

    # ── Log viewer ────────────────────────────────────────────────────────────
    "log.src_journal_all":      {"it": "journalctl (tutto)",       "en": "journalctl (all)",      "de": "journalctl (alles)",      "fr": "journalctl (tout)",        "es": "journalctl (todo)"},
    "log.src_journal_err":      {"it": "journalctl (errori)",      "en": "journalctl (errors)",   "de": "journalctl (Fehler)",     "fr": "journalctl (erreurs)",     "es": "journalctl (errores)"},
    "log.src_journal_svc":      {"it": "journalctl -u <service>",  "en": "journalctl -u <service>","de": "journalctl -u <Dienst>", "fr": "journalctl -u <service>",  "es": "journalctl -u <servicio>"},
    "log.src_tail":             {"it": "tail -f <file>",           "en": "tail -f <file>",        "de": "tail -f <Datei>",         "fr": "tail -f <fichier>",        "es": "tail -f <archivo>"},
    "log.btn_start":            {"it": "▶ Avvia",                  "en": "▶ Start",               "de": "▶ Starten",               "fr": "▶ Démarrer",               "es": "▶ Iniciar"},
    "log.btn_clear":            {"it": "Svuota",                   "en": "Clear",                 "de": "Leeren",                  "fr": "Vider",                    "es": "Vaciar"},
    "log.filter_label":         {"it": "Filtro:",                  "en": "Filter:",               "de": "Filter:",                 "fr": "Filtre :",                 "es": "Filtro:"},
    "log.filter_ph":            {"it": "regex…",                   "en": "regex…",                "de": "Regex…",                  "fr": "regex…",                   "es": "regex…"},
    "log.autoscroll":           {"it": "Auto-scroll",              "en": "Auto-scroll",           "de": "Auto-Scrollen",           "fr": "Défilement auto",          "es": "Desplaz. auto"},
    "log.waiting":              {"it": "In attesa di connessione…","en": "Waiting for connection…","de": "Warte auf Verbindung…",  "fr": "En attente de connexion…", "es": "Esperando conexión…"},
    "log.svc_ph":               {"it": "nome.service",             "en": "service.name",          "de": "dienst.service",          "fr": "service.nom",              "es": "nombre.service"},
    "log.file_ph":              {"it": "/var/log/syslog",          "en": "/var/log/syslog",       "de": "/var/log/syslog",         "fr": "/var/log/syslog",          "es": "/var/log/syslog"},

    # ── Sync directory (WinSCP) ───────────────────────────────────────────────
    "sync.btn":                 {"it": "Sincronizza",              "en": "Synchronize",           "de": "Synchronisieren",         "fr": "Synchroniser",             "es": "Sincronizar"},
    "sync.tooltip":             {"it": "Sincronizza cartella locale ↔ remota", "en": "Synchronize local ↔ remote folder", "de": "Lokalen ↔ Remote-Ordner synchronisieren", "fr": "Synchroniser dossier local ↔ distant", "es": "Sincronizar carpeta local ↔ remota"},
    "sync.title":               {"it": "Sincronizza cartella",     "en": "Synchronize folder",    "de": "Ordner synchronisieren",  "fr": "Synchroniser le dossier",  "es": "Sincronizar carpeta"},
    "sync.local_lbl":           {"it": "<b>Locale:</b>",           "en": "<b>Local:</b>",         "de": "<b>Lokal:</b>",           "fr": "<b>Local :</b>",           "es": "<b>Local:</b>"},
    "sync.remote_lbl":          {"it": "<b>Remoto:</b>",           "en": "<b>Remote:</b>",        "de": "<b>Remote:</b>",          "fr": "<b>Distant :</b>",         "es": "<b>Remoto:</b>"},
    "sync.chk_local_only":      {"it": "Solo locale",              "en": "Local only",            "de": "Nur lokal",               "fr": "Local seulement",          "es": "Solo local"},
    "sync.chk_remote_only":     {"it": "Solo remoto",              "en": "Remote only",           "de": "Nur remote",              "fr": "Distant seulement",        "es": "Solo remoto"},
    "sync.chk_modified":        {"it": "Modificati",               "en": "Modified",              "de": "Geändert",                "fr": "Modifiés",                 "es": "Modificados"},
    "sync.chk_equal":           {"it": "Uguali",                   "en": "Equal",                 "de": "Gleich",                  "fr": "Identiques",               "es": "Iguales"},
    "sync.btn_sel_all":         {"it": "Seleziona tutto",          "en": "Select all",            "de": "Alle auswählen",          "fr": "Tout sélectionner",        "es": "Seleccionar todo"},
    "sync.btn_desel":           {"it": "Deseleziona tutto",        "en": "Deselect all",          "de": "Alle abwählen",           "fr": "Tout désélectionner",      "es": "Deseleccionar todo"},
    "sync.btn_invert":          {"it": "Inverti selezione",        "en": "Invert selection",      "de": "Auswahl umkehren",        "fr": "Inverser la sélection",    "es": "Invertir selección"},
    "sync.col_file":            {"it": "File",                     "en": "File",                  "de": "Datei",                   "fr": "Fichier",                  "es": "Archivo"},
    "sync.col_stato":           {"it": "Stato",                    "en": "Status",                "de": "Status",                  "fr": "État",                     "es": "Estado"},
    "sync.col_l_size":          {"it": "Dim. locale",              "en": "Local size",            "de": "Lok. Größe",              "fr": "Taille locale",            "es": "Tam. local"},
    "sync.col_r_size":          {"it": "Dim. remota",              "en": "Remote size",           "de": "Rem. Größe",              "fr": "Taille distante",          "es": "Tam. remoto"},
    "sync.col_l_mtime":         {"it": "Mod. locale",              "en": "Local mod.",            "de": "Lok. geändert",           "fr": "Modif. locale",            "es": "Mod. local"},
    "sync.col_r_mtime":         {"it": "Mod. remota",              "en": "Remote mod.",           "de": "Rem. geändert",           "fr": "Modif. distante",          "es": "Mod. remota"},
    "sync.col_dir":             {"it": "Direzione",                "en": "Direction",             "de": "Richtung",                "fr": "Direction",                "es": "Dirección"},
    "sync.scanning":            {"it": "Scansione in corso…",      "en": "Scanning…",             "de": "Scanne…",                 "fr": "Analyse en cours…",        "es": "Escaneando…"},
    "sync.stato_local":         {"it": "Solo locale",              "en": "Local only",            "de": "Nur lokal",               "fr": "Local seulement",          "es": "Solo local"},
    "sync.stato_remote":        {"it": "Solo remoto",              "en": "Remote only",           "de": "Nur remote",              "fr": "Distant seulement",        "es": "Solo remoto"},
    "sync.stato_modified":      {"it": "Modificato",               "en": "Modified",              "de": "Geändert",                "fr": "Modifié",                  "es": "Modificado"},
    "sync.stato_equal":         {"it": "Uguale",                   "en": "Equal",                 "de": "Gleich",                  "fr": "Identique",                "es": "Igual"},
    "sync.cancel":              {"it": "Annulla",                  "en": "Cancel",                "de": "Abbrechen",               "fr": "Annuler",                  "es": "Cancelar"},
    "sync.go":                  {"it": "Sincronizza selezionati",  "en": "Sync selected",         "de": "Ausgewählte sync.",       "fr": "Synchroniser sélectionnés","es": "Sincronizar seleccionados"},
    ...
    "settings.general.restore_sessions": {
        "it": "Ripristina le sessioni aperte al prossimo avvio",
        "en": "Restore open sessions on next startup",
        "de": "Geöffnete Sitzungen beim nächsten Start wiederherstellen",
        "fr": "Restaurer les sessions ouvertes au prochain démarrage",
        "es": "Restaurar las sesiones abiertas en el próximo inicio",
    },
    "settings.general.restore_sessions_tt": {
        "it": "Alla chiusura salva la lista delle sessioni aperte e le riapre all'avvio successivo",
        "en": "On close, saves the list of open sessions and reopens them on next startup",
        "de": "Beim Schließen wird die Liste der offenen Sitzungen gespeichert und beim nächsten Start wiederhergestellt",
